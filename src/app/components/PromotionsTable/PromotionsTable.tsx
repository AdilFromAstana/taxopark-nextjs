...
const PromotionsTable: React.FC<PromotionsTableProps> = memo(({ parks }) => {
  ...
  const fetchData = async ({
    filteredTitle = searchTitle,
    filteredParkId = parkId,
  }) => {
    try {
      setIsLoading(true);
      const response = await fetch(
        `${API_URL}/promotions?page=${currentPage}&limit=${limit}&sortField=${sortConfig.key}&sortOrder=${sortConfig.order}&filteredTitle=${filteredTitle}&filteredParkId=${filteredParkId}`
      );
  ...
  return (
    <div className="p-4 h-full flex flex-col gap-4">
      <div className="flex justify-between items-center">
        ...
        <SaveExcelButton
          dataType="parks"
          url={`${API_URL}/parks?page=${currentPage}&limit=${limit}&sortField=${sortConfig.key}&sortOrder=${sortConfig.order}&parkId=${parkId}`}
        />
      </div>
