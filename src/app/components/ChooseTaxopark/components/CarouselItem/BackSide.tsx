...
import { MdBusinessCenter, MdCarRental, MdHeadsetMic, MdPercent } from "react-icons/md";
...
const BackSide: React.FC<BackSideProps> = memo(({ item, toggleFlip, openModal }) => {
                ...
                {details.map(({ icon: Icon, label, value, customIconClass }, idx) => (
                    <div key={idx} className="flex items-center gap-3">
                        <div className="w-6 flex items-start">
                            <Icon className={customIconClass ? "-ml-1 text-2xl" : "text-xl"} />
                        </div>
